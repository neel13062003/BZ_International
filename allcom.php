...
<body class="back">';
    ...
    if($loggedin)
      ...
      <!----main class="container mt-5">
        ...
        ?>
        <table class='table table-striped table-hover text-center' id='NoOrder1' style='font-size:20px;color:black; font-family: Bahnschrift, sans-serif;' >
            <thead> 
                <tr>
                    <th>ID</th>
                    <th>Name</th>
                    <th>TotalInvestment</th>
                    <th>TotalCommision</th>
                </tr>
            </thead>
            <tbody>
        <?php

        // echo "<tr><td>" . $row['id'] . "</td><td>" . $row['name'] . "</td><td>" . $total_investment . "</td></tr>";
        // echo "<a href='commision?name=" . urlencode($row['name']) . "'>Get Total Investment</a>";
        ...
        $database = "mlm";
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $upline = $row['name'];
                                    $FT = TotalI($upline, $conn);
                                    $TI = TotalC($upline, $conn);

                                    echo "<tr>";
                                    echo "<td>" . $row['id'] . "</td>";
                                    echo "<td><a href='commision?name=" . urlencode($row['name']) . "' style='color:blue;'>" . $row['name'] . "</a></td>";
                                    echo "<td>" . $FT . "</td>";
                                    echo "<td>" . $TI . "</td>";
                                    echo "</tr>";
                                }
                            ...
                            function getInvestment($upline, $conn)
                            {
                                $query = "SELECT amount FROM members WHERE name = '$upline'";
